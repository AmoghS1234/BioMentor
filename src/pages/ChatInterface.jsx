...
const MermaidChart = memo(({ chart }) => {
  ...
  useEffect(() => {
    const renderChart = async () => {
      ...
      cleanChart = cleanChart.replace(/```mermaid/i, '').trim();
      
      // 2. Normalize Graph Header
      cleanChart = cleanChart.replace(/^(graph|flowchart)\s+(T[DLRB]|LR|RL|TB|BT)\s*/i, (match, p1, p2) => 
          `${p1.toLowerCase()} ${p2.toUpperCase()}\n` 
      ).trim();
  ...
  return (
    <div className="my-6 rounded-xl overflow-hidden shadow-sm border border-border bg-white">
      ...
      <div 
        className="p-6 flex justify-center overflow-x-auto bg-white" 
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-1 custom-scrollbar">
          {!isAuthReady ? (
            ...
            sessions.map(session => (
              <div key={session.id} onClick={() => setCurrentSessionId(session.id)} className={`group flex items-center justify-between p-3 rounded cursor-pointer transition-all border border-transparent ${session.id === currentSessionId ? 'bg-brand/10 border-brand/20 text-brand' : 'text-txt-secondary hover:bg-page hover:text-txt-primary hover:border-border'}`}>
                <div className="flex items-center gap-3 overflow-hidden"><MessageSquare size={14} className={session.id === currentSessionId ? "fill-current" : ""} /><span className="truncate text-xs font-medium font-mono">{session.title}</span></div>
                <button onClick={(e) => deleteChat(session.id, e)} className="opacity-0 group-hover:opacity-100 hover:text-red-400 transition-opacity p-1.5 hover:bg-red-500/10 rounded"><Trash2 size={12} /></button>
              </div>
            ))
          )}
        </div>
