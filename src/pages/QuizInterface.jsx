...
export default function QuizInterface() {
  ...
  useEffect(() => {
    const loadUserQuizzes = async () => {
      ...
      if (snap.exists()) {
        ...
        // Merge defaults if they haven't been deleted by the user yet
        // logic: we just load what is in DB if it exists, else default
  ...
  const handleGenerate = async () => {
    ...
    newQuizData[customTopic] = generated;
    ...
    if (user) {
      // Save EVERYTHING (including defaults if they exist) to customQuizzes
      // We are treating the entire object as customizable now
      await setDoc(doc(db, 'users', user.uid), { customQuizzes: newQuizData }, { merge: true });
    }
  };
  ...
  const deleteQuiz = async (cat, e) => {
    e.stopPropagation();
    if (!confirm(`Delete the "${cat}" exam?`)) return;

    const newQuizData = { ...quizData };
    delete newQuizData[cat];

    setQuizData(newQuizData);

    if (user) {
      // Overwrite the entire map in Firestore
      await updateDoc(doc(db, 'users', user.uid), { customQuizzes: newQuizData });
    }
  };
      ...
      <div className="space-y-8 animate-fadeIn max-w-5xl mx-auto">
        ...
        <div id="quiz-ai-generator" className="pro-panel p-6 bg-input/10 border-dashed border-2 border-border flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          <div className="flex gap-2 w-full md:w-auto">
            <input value={customTopic} onChange={(e) => setCustomTopic(e.target.value)} placeholder="e.g. 'CRISPR Cas9'..." className="pro-input bg-page w-full md:w-64" onKeyPress={(e) => e.key === "Enter" && handleGenerate()} />
            ...
            <div key={cat} onClick={() => setCategory(cat)} className="pro-panel bg-panel p-6 hover:border-brand hover:bg-brand/5 transition-all cursor-pointer group relative">
              <div className="flex justify-between items-center mb-2">
                ...
                <button 
                  ...
                  className="text-txt-muted hover:text-red-400 p-1 transition-colors z-10"
                  title="Delete Exam"
                >
                  <Trash2 size={16}/>
                </button>
              ...
              <p className="text-sm text-txt-secondary">{quizData[cat]?.length || 0} Questions • Multiple Choice</p>
            </div>
          ))}
        </div>
      </div>
